var app = angular.module('mybanjir', ['ngRoute', 'ngDisqus', 'ui.bootstrap',
        'jmdobry.angular-cache', 'tc.chartjs'
    ]).controller('FeedsController', function($scope, $filter, $routeParams) {
        $scope.param = $routeParams.param ? $routeParams.param.split('::') :
            false;
        $scope.updateFeeds();
        ...
        $scope.setFilteredFeeds = function() {
            ...
            $scope.filteredFeeds = $scope.feeds.sort(function(a, b) {
                return (new Date(a.Timestamp)).getTime() < (new Date(
                    b.Timestamp)).getTime() ? 1 : -1
            }).filter(function(feed) {
                if ($scope.param) {
                    return (feed[$scope.param[0]] || 'others') ===
                        $scope.param[1].replace(/_/g, ' ');
                }
                return true;
            }).slice(begin, end);
            $scope.filteredFeedsLength = $scope.feeds.filter(function(
                feed) {
                if ($scope.param) {
                    return (feed[$scope.param[0]] || 'others') ===
                        $scope.param[1].replace(/_/g, ' ');
        ...
        if ($routeParams.ID) {
            $scope.$watch('feeds.length', function() {
                $scope.feed = $scope.feeds.filter(function(feed) {
                    return feed.ID === parseInt(
                        $routeParams.ID, 10);
                }).pop();
            });
        }
    }).controller('RiverLevelController', function($scope, $routeParams, $http,
        $angularCacheFactory) {
        $scope.state = $scope.riverLevels.filter(function(state) {
            return state.code === $routeParams.state_code;
        }).pop();
        if (!$scope.state.rivers.length) {
            $http.get('http://www.mybanjir.com/riverlevel.aspx?state=' +
                $routeParams.state_code, {
    ...
    }).controller('WeatherForecastController', function($scope, $routeParams,
        $http, $angularCacheFactory, $timeout) {
        ...
        if (!Object.keys($scope.state.forecasts).length) {
            $http.get('http://www.mybanjir.com/met/met.php?states=' +
                $routeParams.state_code, {
                    cache: 3600000
        ...
        $scope.setFilteredFeeds = function() {
            if (Object.keys($scope.state.forecasts).length) {
                $scope.filteredForecasts = $.map($scope.state.forecasts,
                    function(v, k) {
                        if (!$scope.dates.length) $scope.dates = v.results
                            .map(function(result) {
                                return moment(result.date);
                            });
                        v.location = k;
                        if (!$routeParams.selected_date) return $.extend({},
                            v, v.results[0]);
                        return $.extend({}, v, v.results.filter(
                            function(res) {
                                return res.date ===
                                    $routeParams.selected_date;
                            }).pop());
                    });
                var dateTxt = $routeParams.selected_date ? moment(
                    $routeParams.selected_date) : $scope.dates[0];
                $scope.dateTxt = dateTxt.format('dddd LL');
            }
        }
        $scope.classActive = function(date) {
            if (!$routeParams.selected_date) return date === $scope.dates[
                0] ? 'btn btn-primary' : 'btn btn-default';
            return date.format('YYYY-MM-DD') === $routeParams.selected_date ?
                'btn btn-primary' : 'btn btn-default';
        }
        $scope.$watch('state.forecasts', $scope.setFilteredFeeds);
        $scope.dateUrl = function(state, date) {
            return '#!/weatherforecast/' + state.code + '/' + date.format(
                'YYYY-MM-DD');
        }
    }).controller('FloodedRoadController', function($scope, $routeParams, $http,
        $angularCacheFactory) {
        ...
        if (!$scope.state.roads.length) {
            $http.get(
                'http://node.globalresearch.my/mybanjir/ws.BencanaJKR.php?function=getBencanaJKR&output=json&state_code=' +
                $routeParams.state_code, {
                    cache: $angularCacheFactory.get('floodedRoadsCache')
                }).success(function(roads) {
                $scope.state.roads = roads;
                if (roads.length === 0) $scope.error =
                    'No data found'
            }).error(function() {
                $scope.error = 'Feed unavailable at the moment'
            });
        ...
        $scope.$watch('state.roads.length', function() {
            $scope.state = $scope.floodedRoads.filter(function(
                state) {
                return state.code === $routeParams.state_code;
            }).pop();
        });
        ...
        if (!$scope.dropoffPoints.length) {
            $http.get(
                'http://node.globalresearch.my/mybanjir/ws.DropOffPoints.php?function=getDropOffPoints'
    ...
    }).controller('RiverLevelsController', function($scope, $routeParams) {}).controller(
        'WeatherForecastsController', function($scope, $routeParams) {}).controller(
        'FrequencyController', function($scope, $routeParams) {}).controller(
        'FloodedRoadsController', function($scope, $routeParams) {})
    ...
    .filter('fromNow', function($sce) {
        ...
        return function(river) {
            if (river.RiverLevel <= river.AlertLevel) return
                'label-success';
            if (river.RiverLevel <= river.WarningLevel) return
                'label-info';
            if (river.RiverLevel <= river.DangerLevel) return
                'label-warning';
            return 'label-danger';
        };
    }).filter('riverStatus', function() {
        return function(river) {
            if (river.RiverLevel <= river.AlertLevel) return 'Normal';
            if (river.RiverLevel <= river.WarningLevel) return 'Alert';
            if (river.RiverLevel <= river.DangerLevel) return 'Warning';
            return 'Danger';
        };
        ...
        $rootScope.updateFeeds = function() {
            $http.get('http://www.mybanjir.com/news.aspx').success(
                function(datas) {
                    $rootScope.feeds = datas;
                    $rootScope.locations = datas.map(function(data) {
                        if (!data.Location) return 'others'
        ...
        $rootScope.slugify = function(str) {
            ...
            str = str.replace(/[^a-z0-9 -]/g, '').replace(/\s+/g, '-').replace(
                /-+/g, '-');
            var rand = '',
                chars = '0123456789abcdefghijklmnopqrstuvwxyz';
            for (var i = 4; i > 0; --i) rand += chars[Math.round(Math.random() *
                (chars.length - 1))];
            return str + '-' + rand;
        };
...
(function() {
    ...
    setInterval(function() {
        if ((window.Firebug && window.Firebug.chrome && window.Firebug
                .chrome.isInitialized) || window.outerWidth -
            window.innerWidth > threshold || window.outerHeight -
...
window.addEventListener('devtoolschange', function(e) {
    console.log(
        'Care to help? Fork us on github: https://github.com/zulfajuniadi/mybanjirinfo'
    );
});